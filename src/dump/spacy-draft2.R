...
g <- core %>%  
  ggplot(.) +
  stat_bin_hex(aes(x=x, y=y), bins =45) +
...
ggsave('logo-triple-hex-clear-poster2.png', g, height=15, width=15, bg='transparent')

g

q <- ggplot() +
  stat_bin_hex(data=core, aes(x=x, y=y), bins =53) +
  # stat_density_2d(aes(x=x, y=y, fill=..level..), geom = 'polygon') +
  # geom_density_2d(aes(x=x, y=y, fill=..level..), alpha=0.5, colour='white') +
  # geom_point(aes(x=x, y=y), alpha=0.008, color='white') +
  geom_text(data=tibble(x=mean(c(-20, 120)), y=-10), aes(x=x, y=y, label='<Digital Democracy Lab>'), col='white', size=8, family='Source Code Pro Semibold') +
  scale_x_continuous(limits = c(-20, 120), expand = c(0,0)) +
...
q
bg <- ggplot() +
  scale_x_continuous(limits = c(-100, 220), expand = c(0,0)) +
  scale_y_continuous(limits = c(-320, 420), expand = c(0,0)) +
  theme(
    panel.background = element_rect(fill='black'),
    panel.grid = element_blank(),
    axis.text = element_blank(),
    ...
    axis.title = element_blank(),
    panel.border = element_blank(),
    plot.margin = margin(0, 0, -0.1, -1, "cm")
  )

g <- ggplotGrob(q)

l <- bg+annotation_custom(grob = g)
l
ggsave(filename = '/Users/lucienbaumgartner/ddl-git/designs/banner-hex.png', l, width=35)
ggplot() +
  # stat_bin_hex(aes(x=x, y=y), bins =45) +
  # stat_density_2d(aes(x=x, y=y, fill=..level..), geom = 'polygon') +
  # geom_density_2d(aes(x=x, y=y, fill=..level..), alpha=0.5, colour='white') +
  # geom_point(aes(x=x, y=y), alpha=0.008, color='white') +
  geom_text(aes(x=20, y=10, label='<Digital Democracy Lab>'), col='white', size=1) +
  scale_x_continuous(limits = c(-120, 220), expand = c(0,0)) +
  scale_y_continuous(limits = c(-20, 120), expand = c(0,0)) 
g

#AB1A12

# ggsave('logo-triple-hex2.png', g, height=15, width=15)

g <- core %>%  
  ggplot(.) +
  stat_bin_hex(aes(x=x, y=y), bins =40, alpha=0.8) +
  # stat_density_2d(aes(x=x, y=y, fill=..level..), geom = "polygon") +
  # geom_density_2d(aes(x=x, y=y), alpha=0.2, colour='white') +
  # geom_point(data=aes(x=x, y=y), alpha=0.01, color='white') +
  scale_x_continuous(limits = c(-20, 120), expand = c(0,0)) +
  scale_y_continuous(limits = c(-20, 120), expand = c(0,0)) +
...
# ggsave('logo-triple-raster.png', g, height=15, width=15)
