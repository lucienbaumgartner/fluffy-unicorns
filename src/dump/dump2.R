...
vec <- NA
for(i in 1:nrow(df)){
  if(i==1){vec[i] <- 11}else{
    if(identical(df$chars[i], "")){vec[i] <- vec[i-1]+16}else{
      if(identical(df$chars[i], 'a')&identical(df$chars[i-1], 'l')){vec[i] <- vec[i-1]+15}else{
        if(identical(df$chars[i], 'r')){vec[i] <- vec[i-1]+12}else{
          if(df$chars[i-1]%in%c('e', 'm', 'o', 'c', 'd')){vec[i] <- vec[i-1]+20}else{
            if(identical(df$chars[i], 'a')){vec[i] <- vec[i-1]+14}else{
              vec[i] <- vec[i-1]+13
            }
          }
        }
...
chars2data <- function(char, anker, ylims, index){
  ...
  if(identical(char, 't')){
    tbig <- tibble(y=jitter(rep(seq(ylims[1], ylims[2], 0.2), 20), factor = 15), 
                   x=jitter(rep(anker, length(rep(seq(ylims[1], ylims[2], 0.2), 20))), factor = 1-(1-(1/index))),
                   lB= 'char_tbig')
    tsmall <- tibble(y=jitter(rep(max(ylims), length(rep(seq(anker-1.5, anker+1.5, 0.02), 20))), factor = 1-(1-(1/index))),
                     x=jitter(rep(seq(anker-1.5, anker+1.5, 0.02), 20), factor = 15),
                     lB= 'char_tsmall')
    return(rbind(tbig, tsmall))
  }
  if(identical(char, 'a')){
    aleft <- tibble(y=jitter(rep(seq(ylims[1], ylims[2], 0.3), 35), factor = 10), 
                    x=jitter(rep(seq(anker-1, anker, 0.015), 35), factor = 10),
                    lB='char_aleft') 
    aright <- tibble(y=jitter(rep(seq(ylims[2], ylims[1], -0.3), 35), factor = 10), 
                     x=jitter(rep(seq(anker, anker+1, 0.015), 35), factor = 10),
                     lB='char_aright')
  ...
  if(identical(char, 'l')){
    lbig <- tibble(y=jitter(rep(seq(ylims[1], ylims[2], 0.2), 20), factor = 15), 
                   x=jitter(rep(anker, length(rep(seq(ylims[1], ylims[2], 0.2), 20))), factor = 1-(1-(1/index))),
                   lB= 'char_lbig')
    lsmall <- tibble(y=jitter(rep(min(ylims), length(rep(seq(anker, anker+2, 0.02), 20))), factor = 1-(1-(1/index))), 
                     x=jitter(rep(seq(anker, anker+2, 0.02), 20), factor = 1),
                     lB= 'char_lsmall')
  ...
  if(identical(char, 'm')){
    ...
    msmallleft <- tibble(y=jitter(rep(seq(ylims[2], median(ylims), -0.3), 35), factor = 10), 
                         x=jitter(rep(seq(anker-1, anker, 0.03), 35), factor = 40-index*2),
                         lB='char_aright')
    
    msmallright <- tibble(y=jitter(rep(seq(median(ylims), ylims[2], 0.3), 35), factor = 10), 
                          x=jitter(rep(seq(anker, anker+1, 0.03), 35), factor = 40-index*2),
                          lB='char_aleft') 
  ...
  if(identical(char, 'e')){
    estring <- tibble(y=jitter(rep(seq(ylims[1], ylims[2], 0.2), 20), factor = 15), 
                      x=jitter(rep(anker, length(rep(seq(ylims[1], ylims[2], 0.2), 20))), factor = 1-(1-(1/index))),
                      lB= 'char_estring')
    ebiglower <- tibble(y=jitter(rep(min(ylims), length(rep(seq(anker, anker+2, 0.02), 20))), factor = 1-(1-(1/index))), 
                        x=jitter(rep(seq(anker, anker+2, 0.02), 20), factor = 1),
                        lB= 'char_ebiglower')
    ebigupper <- tibble(y=jitter(rep(min(ylims), length(rep(seq(anker, anker+2, 0.02), 20))), factor = 1-(1-(1/index))), 
                        x=jitter(rep(seq(anker, anker+2, 0.02), 20), factor = 1),
                        lB= 'char_ebigupepr') %>% 
      mutate(y=y+(ylims[2]-ylims[1]))
    esmall <- tibble(y=jitter(rep(min(ylims), length(rep(seq(anker, anker+1, 0.02), 20))), factor = 1-(1-(1/index))), 
                     x=jitter(rep(seq(anker, anker+1, 0.02), 20), factor = 1),
                     lB= 'char_esmall') %>% 
  ...
  if(identical(char, 'y')){
    ybig <- tibble(y=jitter(rep(seq(ylims[1], ylims[2], 0.1), 15), factor = 10), 
                   x=jitter(rep(seq(anker-2, anker+1, 0.015), 15), factor = 10),
                   lB='char_ybig')
    ysmall <- tibble(y=jitter(rep(seq(ylims[2], mean(ylims), -0.15), 15), factor = 10), 
                     x=jitter(rep(seq(anker-2, anker, 0.03), 15), factor = 10),
                     lB='char_ysmall') %>% 
      mutate(x=x-2)
  ...
  if(identical(char, 'r')){
    ...
    rsmall <- tibble(y=jitter(rep(seq(mean(ylims), ylims[1], -0.15), 15), factor = 10), 
                     x=jitter(rep(seq(anker, anker+2, 0.03), 15), factor = 10),
                     lB='char_rsmall') %>% 
      mutate(x=x-2)
...
rndm <- rndm <- sapply(tibble(y=rnorm(500000, mean = mean(ylims), sd = 20), 
                              x=rnorm(500000, mean = mean(c(min(l$x), max(l$x))), sd = 10)), function(x) jitter(rep(x, 5), factor=50)) %>% 
  as_tibble %>% 
  mutate(lB='char_rndm')
rndm2 <- rndm <- sapply(tibble(y=rnorm(5000, mean = mean(ylims), sd = 20), 
                               x=rnorm(5000, mean = mean(c(min(l$x), max(l$x)))-5, sd = 10)), function(x) jitter(rep(x, 5), factor=50)) %>% 
  as_tibble %>% 
  mutate(lB='char_rndm')
rndm3 <- rndm <- sapply(tibble(y=rnorm(5000, mean = mean(ylims), sd = 20), 
                               x=rnorm(5000, mean = mean(c(min(l$x), max(l$x)))+5, sd = 10)), function(x) jitter(rep(x, 5), factor=50)) %>% 
  as_tibble %>% 
  mutate(lB='char_rndm')
...
ggplot(l) +
  ...
  #stat_bin_hex(aes(x=x, y=y), bins =50) +
